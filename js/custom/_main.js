...
// Fallback: hide preloader after 4s so we never get stuck if window.load never fires (e.g. broken/missing image)
setTimeout(function() {
    "use strict";
    jQuery(".preloader").length && preloader();
}, 4000);
		...
		if(tpj('#rev_slider_1').revolution == undefined){
			revslider_showDoubleJqueryError('#rev_slider_1');
		   	...
		   	revapi4 = tpj('#rev_slider_1').show().revolution(
				...
				// Hide preloader when slider is ready (hero no longer "loading")
				tpj('#rev_slider_1').on('revolution.slide.onloaded', function() {
					preloader();
				});
			}
		});
